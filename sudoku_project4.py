...
while(1):
	...
	for ctr in contours:
		...
		if(rect[2]*rect[3] > max_area):
			...
			max_area_rect = rect
			max_area_ctr = ctr
	# if(abs(max_area_rect[2] - max_area_rect[3]) > 50 
	# 		or max_area_rect[3] < 280 or max_area_rect[2] < 280):
	# 	continue
...
# =========================================================================================
		...
		# print(width, height)
		if(height > 220 and width > 220):# or abs(height - width) > 50):
			...
			for rect in rects:
				...
				if(out.shape[1] < rect[1] or out.shape[0] < rect[0]):
					continue
				# put_digits(sudoku, pred_array[N], out.shape[1], out.shape[0], rect[1], rect[0])
				N += 1
			if(N < 17):
				print("not enough")
				continue
			...
			# print_sudoku(sudoku)
			# t = time.time()
			# b = solve(sudoku_copy, 0, 0, sudoku_copy[0][0], t)
			# print("after solve")
			# print_sudoku(b)
			if(str(b) == 'False'):
				continue
			...
			# 	for j in range(9):
			# 		if(not sudoku[i][j]):
			# 			print("putting", b[i][j], i, j)
			# 			cv2.putText(out, str(b[i][j]), (int((j+1/3)*out.shape[0]//9), int((i+2/3)*out.shape[1]//9)), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 1)
	...
	if(str(out) == "0"):
		continue
	# print(out.shape[0], out.shape[1], approx)
	cv2.imshow("out", out)
	cv2.imshow("title", im)
	# cv2.imshow("thresh", thresh)
